...
The rotation matrix R can be used to describe a rotation from a body-fixed frame to the inertial frame [CITE THIS].
...
In these elaborate systems, constraint forces can be implemented to eliminate degrees of freedom and reduce the complexity of the system [CITE THIS]. Additionally, Lagrangian equations are invariant to changes of coordinate systems [CITE THIS].
\par
The Lagrangian function is represented by L, and is defined as the difference between kinetic and potential energies in a system modeled using positions and velocities [CITE THIS].
...
\end{equation}
The Euler-Lagrange equations are equivalent to Newton's second law [CITE THIS], and are implicit second-order differential equations applied to a given coordinate system [CITE THIS].
\par
...
Nonholonomic constraints can be implemented to restrict velocities in a system [CITE THIS].
Lagrange multipliers can be added to the unconstrained equations of motion to represent unknown constraint forces [CITE THIS].
\par
...
Numerical Integration allows for the approximate computation of an integral using numerical methods [CITE THIS].
When applying numeric integration to a differential system, stiffness is often a byproduct. 
