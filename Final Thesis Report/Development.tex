...
With the coordinate system defined, Euler angles were implemented to represent the roll ($\alpha$), pitch ($\psi$), and yaw ($\theta$) of the system.
\par
...
\end{figure}
\par
The pitch angle ($\psi$) of the system is a rotation about the $Y$-axis. This represents a rotation about the Y axis. The wheels are completely unbounded in their ability to rotate, and can pivot through [0, 2$\pi$].
\begin{figure}[!htb]
	\centering
	\minipage{0.5\textwidth}
...
\end{figure}
\par
The yaw angle ($\theta$) is the angle of the system about the $Z$-axis (right hand rule applied). There are two yaw angles on the RipStik, one for the front caster ($\theta_{fc}$), and the second for the back caster ($\theta_{bc}$).
...
In order to validate the selected coordinate system, a test case was conducted for the RipStik. This was done by analyzing the behaviour of the caster as it completed a full revolution (2$\pi$ radians).
...
In addition to the two-dimensional plot, a three-dimensional plot was generated to verify that the casters' behaviour aligned with expectations based on experimentation with a physical RipStik. The three-dimensional plot is shown in Figure \ref{fig:CasterWheel3DTest}.
...
Therefore, the selected coordinate system is valid, meaning the equations of motion can be developed.
