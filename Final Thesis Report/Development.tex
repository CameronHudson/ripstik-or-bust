...
As discussed in the technical background, four methods for numerically integrating stiff DAE systems were analyzed. 
...
Using the Collocation integration method, 5.8 seconds of output for the falling RipStik was plotted for each degree of freedom in the system. The output can be seen in Figure 
...
The first concern when implementing LQR control was the effect of the nonholonomic constraints on the linearization process. This was done using the result discussed in section \ref{sec:linnonholo}. To verify that this process worked with the control forces for a simple example, a test case was developed by combining two previous examples.
