...
Additionally, the casters were modelled as skates to emulate the non-slip behaviour of the wheels without consideration for the rolling dynamics. 
The rolling dynamics of the wheels were determined to be inconsequential to the modelling process for the purpose of developing a control system.
\par
The spring in the torsion rod was omitted as it serves only to aid in rider comfort, and as such is not a critical component of the RipStik's dynamics. 
Including the rod will add unnecessary complexity to the system in the form of kinetic energy.
Friction between the bodies of the RipStik was omitted as it is difficult to accurately model and has negligible impact on the general dynamics of the system.

\subsection{Coordinate Systems}

The initial focus for developing a mathematical model of the RipStik was determining the number of degrees of freedom in the system. 
...
\par
A body-fixed coordinate system for the RipStik was developed with the origin was placed at the center of the torsion bar. 
The $x$-$y$ plane is located parallel to the RipStik plates at rest with no torsion applied.
The $+x$ direction points through the torsion bar towards the front plate. The $x$ axis is normal to the $x$-$y$ plane, with $+z$ pointing upwards. The $+y$ direction is defined based on the right hand rule.
...
The roll angles ($\alpha$) describe the rotation of the deck platforms about the $X$-axis. There are two roll angles on the RipStik, one for the front platform ($\alpha_{fp}$) and the second for the rear platform ($\alpha_{bp}$).
\begin{figure}[!htb]
	\centering
	\minipage{0.5\textwidth}
...
The yaw angle ($\theta$) is the angle of the system about the $z$-axis (right hand rule applied). There are two yaw angles on the RipStik, one for the front caster ($\theta_{fc}$), and the second for the back caster ($\theta_{bc}$). The wheels are completely unbounded in their ability to rotate, and can pivot through [0, 2$\pi$].
\begin{figure}[!htb]
	\centering
	\minipage{0.5\textwidth}
...
In order to validate the selected coordinate system, a test case was conducted for the RipStik. This was done by analyzing the behavior of the caster as it completed a full revolution (2$\pi$ radians).
The output for the $x$ ,$y$, and $z$ positions of the contact point at the bottom of the caster were plotted as a function of the casters rotation, as seen in Figure \ref{fig:CasterWheel2DTest}.
...
In addition to the two-dimensional plot, a three-dimensional plot was generated to verify that the casters' behavior aligned with expectations based on experimentation with a physical RipStik. The three-dimensional plot is shown in Figure \ref{fig:CasterWheel3DTest}.
...
With the test case completed and verified, the coordinate system for the RipStik model was verified for each degree of freedom in the system in the same manner as the caster unit test. 
This was completed by applying the output from Mathematica into the ThreeJS based visualization tool.
\subsubsection{Validation}
Each degree of freedom in the system behaved as expected. 
This demonstrates that the position vectors and rotation matrices were properly defined, meaning the equations of motion can be developed.
...
With the test case complete, the equations of motion were tested on the RipStik model. The $Z$ coordinate in the centre of the torsion rod of the RipStik was fixed.
An initial roll angle of $\frac{\pi}{4}$ was applied to the front plate ($\alpha_{fp}$), and an initial roll angle of $-\frac{\pi}{4}$ was applied to the back plate ($\alpha_{bp}$). 
...
Since the plates and casters both experienced oscillations approximately between  $\frac{\pi}{4}$ and $-\frac{\pi}{4}$, it can be confirmed that the equations of motion for the RipStick are accurate.

An accurate visualization of the Ripstik was produced to model the motion. 
The two extreme positions of the RipStik can be seen in figure \ref{fig:RipStikModel1}.
...
With the unconstrained equations of motion validated, nonholonomic constraints were then implemented into the system. 
...
The initial method explored involved symbolically inverting matrices in Mathematica. First, the acceleration terms in the equations of motion were isolated. 
They were then substituted into the derivatives of the nonholonomic constraint equations, shown in Equation \ref{eq:SMI}.
...
\par
At this point, all symbolic constants were replaced with measured values to further simplify computations.
...
\par
Figure \ref{fig:stiffsystem} can be used to aid in building understanding of the behaviour of stiff systems. 
\begin{figure}[!htb]
	\centering
	\minipage{0.7\textwidth}
...
To try and reduce this computational complexity, four different numerical methods were selected to evaluate stiff differential algebraic equation (DAE) systems.
...
As previously discussed, four methods for numerically integrating stiff DAE systems were analysed. 
...
The Collocation method was able to produce an output 9.7 times longer than all other methods, at a cost of 22 times longer computation.  
...
Using the Collocation integration method, 5.8 seconds of output for the falling RipStik was generated for each degree of freedom in the system. 
As the RipStik fell, it eventually exceeded the range of validity for the Euler angles. 
This led to the RipStiks motion behaving erratically. 
...
\par
An accurate visualization was produced to model the motion of the RipStik. The initial upright and final fallen over position of the RipStik are shown in Figure \ref{fig:fallcaster}
...
Note that the pendulum is again modelled as a thin uniform rod.
